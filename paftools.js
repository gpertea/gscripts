...
var paftools_version = '2.17-r949-geomod';
...
function paf_liftover(args)
{
	function read_bed(fn, to_merge)
	{
		...
		while (file.readline(buf) >= 0) {
			...
			var iid=t.length>3 ? t[3] : null;
			bed[t[0]].push([parseInt(t[1]), parseInt(t[2]), iid]);
		}
		buf.destroy();
		file.close();
	...
	if (args.length - getopt.ind < 2) {
		...
		print("Options:");
		print("  -m        merge overlapping query intervals");
	...
	while (file.readline(buf) >= 0) {
		...
		for (var i = 0; i < r.length; ++i) {
			...
			if (regs[i].length>3) name=regs[i][2]+'|'+name
			//truncation status: at 5' (left) and/or 3' (right) end
